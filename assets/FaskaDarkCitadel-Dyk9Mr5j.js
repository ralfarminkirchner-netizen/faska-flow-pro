...
import { $ as MathUtils, Rt as Vector3, St as SRGBColorSpace, a as useFrame, s as useThree, t as Canvas, y as CanvasTexture } from "./react-three-fiber.esm-pJsmxxS9.js";
//#region src/components/games/engines/FaskaDarkCitadel/FaskaDarkCitadel.jsx
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
var import_jsx_runtime = require_jsx_runtime();
var ARENA_LIMIT = 18;
var STORAGE_KEY = "faska-dark-citadel-best-v1";
var SUBJECTS = {
	deutsch: {
		label: "Deutsch",
		color: "#f97316",
		challenges: [
			...
			{
				prompt: "Welcher Artikel passt zu \"Burg\"?",
				answer: "Die",
				options: [
					"Der",
					"Die",
			...
			{
				prompt: "Welche Silbe beginnt \"Ritter\"?",
				answer: "Rit",
				options: [
					"Rit",
					"ter",
			...
			{
				prompt: "Was reimt sich auf \"Licht\"?",
				answer: "Sicht",
				options: [
					"Sicht",
					"Burg",
	...
	mathe: {
		...
		challenges: [
			...
			{
				prompt: "Welche Zahl ist ungerade?",
				answer: "17",
				options: [
					"16",
					"20",
	...
	sach: {
		...
		challenges: [
			...
			{
				prompt: "Wo speichern Pflanzen Wasser?",
				answer: "Wurzeln",
				options: [
					"Wurzeln",
					"Fenster",
	...
	musik: {
		...
		challenges: [
			...
			{
				prompt: "Was gehoert zum Rhythmus?",
				answer: "Pause",
				options: [
					"Pause",
					"Wolke",
...
function shuffle(items, seed) {
	return [...items].map((item, index) => ({
		item,
		order: (index * 91 + seed * 53 + String(item).length * 17) % 997
...
function challengeFor(subject, seed) {
	...
	return {
		...base,
		options: shuffle(base.options, seed).slice(0, 4)
	};
}
...
function spawnWave(level, learnMode, subject, score) {
	...
	const baseOptions = learnMode ? challenge.options : [
		"Ember",
		"Ash",
		"Iron",
		"Veil"
	...
	return {
		enemies: (bossWave ? [Math.PI] : [
			-2.4,
			-.8,
			.8,
			2.4
		]).map((angle, index) => {
			const type = bossWave ? "boss" : choose([
				"hunter",
				"knight",
				"caster",
				"hunter"
			...
			return {
				...
				pos: new Vector3(Math.sin(angle) * (bossWave ? 12 : 14.5), 0, Math.cos(angle) * (bossWave ? 12 : 14.5)),
				yaw: angle + Math.PI,
				hp: Math.round(template.hp + level * (bossWave ? 22 : 7)),
				maxHp: Math.round(template.hp + level * (bossWave ? 22 : 7)),
				cooldown: 2.8 + index * .65,
				windup: 0,
				stun: 0,
				deadTimer: 0,
				attackFlash: 0
...
function consume(input, names) {
	for (const name of names) if (input.pressed[name]) {
...
function damageEnemy(state, enemy, damage, heavy) {
	...
	if (state.learnMode && !enemy.correct) {
		...
		state.particles.push({
			pos: enemy.pos.clone().add(new Vector3(0, 1.2, 0)),
			color: "#fb7185",
			life: .55,
			size: .35
		});
	...
	if (enemy.hp <= 0) {
		...
		state.particles.push({
			pos: enemy.pos.clone().add(new Vector3(0, 1.4, 0)),
			color: "#34d399",
			life: .9,
			size: .72
		});
...
function tryPlayerAttack(state, heavy) {
	...
	player.stamina -= cost;
	player.attackCooldown = heavy ? .86 : .42;
	player.attackTimer = heavy ? 0 : .28;
	player.heavyTimer = heavy ? .48 : 0;
	const range = heavy ? 3 : 2.25;
	const arc = heavy ? 1.18 : .88;
...
function updatePlayer(state, input, dt) {
	...
	if (x || z) {
		const forward = new Vector3(Math.sin(state.cameraYaw), 0, -Math.cos(state.cameraYaw));
		const right = new Vector3(Math.cos(state.cameraYaw), 0, Math.sin(state.cameraYaw));
	...
	if (consume(input, [" ", "shift"]) && player.stamina >= 28) {
		...
		state.particles.push({
			pos: player.pos.clone().add(new Vector3(0, .5, 0)),
			color: "#d946ef",
			life: .35,
			size: .28
		});
	}
	if (movement.lengthSq() > 0 && player.dodge <= 0 && player.attackCooldown < .36) {
...
function updateEnemies(state, dt) {
	...
	if (state.lockOn) {
		if (!aliveEnemies.find((enemy) => enemy.id === state.lockTargetId) && aliveEnemies.length) {
	...
	for (const enemy of state.enemies) {
		...
		if (enemy.type === "caster" && distance < 4.4) {
			...
			enemy.pos.addScaledVector(away, template.speed * .55 * dt);
		} else if (distance > template.range + .32) {
		...
		if (enemy.cooldown <= 0 && distance <= template.range + (enemy.type === "caster" ? 4.5 : .25)) {
			enemy.windup = enemy.type === "boss" ? .62 : .42;
		...
		if (enemy.windup > 0) {
			enemy.windup -= dt;
			if (enemy.windup <= 0) {
				const hitRange = template.range + (enemy.type === "caster" ? 4.5 : .35);
				if (flatDistance(enemy.pos, player.pos) <= hitRange && player.invincible <= 0) {
					...
					state.particles.push({
						pos: player.pos.clone().add(new Vector3(0, 1.2, 0)),
						color: "#ef4444",
						life: .45,
						size: .42
					});
				} else state.message = "Ausgewichen.";
			}
		}
	}
}
...
function ThirdPersonCamera({ stateRef, inputRef }) {
	...
	useFrame((_, delta) => {
		...
		if (state.lockOn && target) state.cameraYaw = MathUtils.lerp(state.cameraYaw, yawTo(state.player.pos, target.pos), Math.min(1, delta * 3.4));
		else {
		...
		const desired = state.player.pos.clone().addScaledVector(forward, -8.4).add(new Vector3(0, 4.6, 0));
...
function PlayerActor({ stateRef }) {
	...
	const swordRef = (0, import_react.useRef)(null);
	const cloakRef = (0, import_react.useRef)(null);
	useFrame((clockState) => {
		const state = stateRef.current;
		...
		const pulse = Math.sin(clockState.clock.elapsedTime * 11) * .04;
		groupRef.current.scale.setScalar(player.invincible > 0 ? 1 + pulse : 1);
		if (swordRef.current) {
			const swing = player.heavyTimer > 0 ? player.heavyTimer / .48 : player.attackTimer / .28;
			swordRef.current.rotation.x = -.8 + (1 - swing) * 1.55;
			swordRef.current.rotation.z = player.heavyTimer > 0 ? -.75 : -.35;
		}
		if (cloakRef.current) cloakRef.current.rotation.x = -.12 + Math.sin(clockState.clock.elapsedTime * 5) * .04;
	});
...
function EnemyActor({ enemyView, stateRef, learnMode }) {
	...
	useFrame((clockState, delta) => {
		const enemy = stateRef.current?.enemies.find((item) => item.id === enemyView.id);
		...
		const aliveScale = enemy.hp > 0 ? 1 : Math.max(.04, enemy.deadTimer / .8);
		...
		if (bodyRef.current) bodyRef.current.position.y = .75 + Math.sin(clockState.clock.elapsedTime * 4 + enemy.pos.x) * .035;
	});
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("group", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("mesh", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("capsuleGeometry", { args: [
					type.radius,
					enemyView.type === "boss" ? 1.6 : .9,
					8,
					16
				] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("meshStandardMaterial", {
					color: type.color,
					roughness: .65,
					metalness: enemyView.type === "knight" ? .55 : .12,
					emissive: enemyView.correct && learnMode ? "#064e3b" : "#000000",
					emissiveIntensity: enemyView.correct && learnMode ? .25 : 0
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("mesh", {
				castShadow: true,
				position: [
					0,
					enemyView.type === "boss" ? 2.1 : 1.55,
					0
				],
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("sphereGeometry", { args: [
					enemyView.type === "boss" ? .42 : .26,
					16,
					16
				] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("meshStandardMaterial", {
					color: "#f8fafc",
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("mesh", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("boxGeometry", { args: [
					...
					.1,
					.08
				] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("meshBasicMaterial", { color: "#111827" })]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("mesh", {
				position: [
					-.64 + hpRatio * .64,
					enemyView.type === "boss" ? 2.86 : 2.26,
					.01
				],
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("boxGeometry", { args: [
					1.28 * hpRatio,
					.12,
					.09
				] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("meshBasicMaterial", { color: enemyView.type === "boss" ? "#dc2626" : "#f59e0b" })]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(LabelSprite, {
				text: enemyView.label,
				color: labelColor,
				position: [
					0,
					enemyView.type === "boss" ? 3.25 : 2.58,
					0
				],
				size: enemyView.type === "boss" ? .72 : .48
			})
		]
	});
}
function CitadelWorld() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
		...
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)("ambientLight", { intensity: .62 }),
		...
		[
			...
			[0, -17],
			[0, 17]
		].map(([x, z]) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("pointLight", {
			position: [
				x,
				2.7,
				z
		...
		/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("mesh", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("circleGeometry", { args: [22, 96] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("meshStandardMaterial", {
				color: "#263244",
				roughness: .82,
				metalness: .08,
				emissive: "#0f172a",
		...
		columns.map(({ x, z }) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("group", {
			position: [
				x,
				0,
				z
		...
		[
			...
			Math.PI,
			Math.PI * 1.5
		].map((rotation) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("group", {
			rotation: [
				0,
				rotation,
				0
...
function ParticleField({ stateRef }) {
	...
	useFrame((_, delta) => {
		...
		if (timerRef.current > .05) {
			timerRef.current = 0;
			setParticles(stateRef.current?.particles.map((particle, index) => ({
				...particle,
				id: index
			})) || []);
		}
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("group", { children: particles.map((particle) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("mesh", {
...
function GameLoop({ stateRef, inputRef, setView, setBest }) {
	const viewTimer = (0, import_react.useRef)(0);
	useFrame((_, delta) => {
		const state = stateRef.current;
		if (!state) return;
...
function DarkCitadelScene({ stateRef, inputRef, view, setView, setBest }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CitadelWorld, {}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ThirdPersonCamera, {
			stateRef,
			inputRef
		}),
		...
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PlayerActor, { stateRef }),
		view.enemies.map((enemy) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EnemyActor, {
			enemyView: enemy,
			stateRef,
			learnMode: view.learnMode
		}, enemy.id)),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ParticleField, { stateRef })
	] });
}
function Bar({ label, value, max, color }) {
	const ratio = max > 0 ? clamp(value / max, 0, 1) : 0;
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "mb-1 flex justify-between text-[10px] font-black uppercase tracking-[.16em] text-white/45",
...
function LabelSprite({ text, color = "#ffffff", position, size = 1 }) {
	...
	(0, import_react.useEffect)(() => () => texture.dispose(), [texture]);
	useFrame(({ camera }) => {
...
function FaskaDarkCitadel({ onExit, isLearncade = false }) {
	const [learnMode, setLearnMode] = (0, import_react.useState)(isLearncade);
	const [subject, setSubject] = (0, import_react.useState)("deutsch");
	const [best, setBest] = (0, import_react.useState)(() => Number(localStorage.getItem(STORAGE_KEY) || 0));
	const bestRef = (0, import_react.useRef)(best);
	const inputRef = useCombatInput();
	const [initialState] = (0, import_react.useState)(() => createGameState({
		learnMode: isLearncade,
		subject: "deutsch",
		best: Number(localStorage.getItem(STORAGE_KEY) || 0)
	}));
	const stateRef = (0, import_react.useRef)(initialState);
	const [view, setView] = (0, import_react.useState)(() => makeSnapshot(initialState));
	(0, import_react.useEffect)(() => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Canvas, {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DarkCitadelScene, {
					stateRef,
					inputRef,
					view,
					setView,
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "pointer-events-none absolute inset-x-0 top-0 z-20 bg-gradient-to-b from-black/80 to-transparent px-5 py-4",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex flex-wrap items-start justify-between gap-4",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "text-[10px] font-black uppercase tracking-[.24em] text-rose-300/70",
							children: "Premium Combat Vertical"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
							className: "text-3xl font-black leading-none md:text-5xl",
							children: "FASKA Dark Citadel"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "mt-2 max-w-xl text-sm font-bold text-white/62",
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								...
								className: `rounded-2xl px-4 py-2 text-sm font-black transition ${!learnMode ? "bg-amber-400 text-slate-950" : "bg-white/10 text-white hover:bg-white/15"}`,
								children: "Normal"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								type: "button",
								onClick: () => setLearnMode(true),
								className: `rounded-2xl px-4 py-2 text-sm font-black transition ${learnMode ? "bg-pink-500 text-white" : "bg-white/10 text-white hover:bg-white/15"}`,
								children: "Lernen"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("select", {
								...
								children: Object.entries(SUBJECTS).map(([id, item]) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
									value: id,
									children: item.label
								}, id))
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								type: "button",
								onClick: reset,
								className: "rounded-2xl bg-white/10 px-4 py-2 text-sm font-black hover:bg-white/15",
								children: "Reset"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								type: "button",
								onClick: onExit,
								className: "rounded-2xl bg-rose-600 px-4 py-2 text-sm font-black text-white shadow-lg shadow-rose-950/40 hover:bg-rose-500",
								children: "Beenden"
							})
						]
					})]
			...
			learnMode && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "pointer-events-none absolute left-1/2 top-28 z-20 w-[min(760px,calc(100vw-2rem))] -translate-x-1/2 rounded-2xl border border-white/10 bg-slate-950/78 px-5 py-3 text-center shadow-2xl backdrop-blur",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-[10px] font-black uppercase tracking-[.22em]",
					style: { color: SUBJECTS[subject].color },
					children: SUBJECTS[subject].label
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "pointer-events-none absolute bottom-4 left-4 z-20 grid w-[min(360px,calc(100vw-2rem))] gap-3 rounded-2xl border border-white/10 bg-slate-950/72 p-4 shadow-2xl backdrop-blur",
				children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "grid grid-cols-3 gap-2 text-xs font-black text-white/70",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["Level ", view.level] }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["Score ", view.score] }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["Best ", Math.max(best, view.score)] })
						]
					})
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "pointer-events-none absolute bottom-4 right-4 z-20 hidden max-w-xl flex-wrap justify-end gap-2 md:flex",
				children: CONTROL_HELP.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "rounded-full border border-white/10 bg-slate-950/62 px-3 py-1 text-[11px] font-black text-white/58 backdrop-blur",
					children: item
				}, item))
			}),
			view.bossWave && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "pointer-events-none absolute inset-x-0 top-24 z-10 text-center",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-sm font-black uppercase tracking-[.36em] text-rose-300",
					children: "Boss"
				})
			})
		]
...
//#endregion
export { FaskaDarkCitadel as default };
