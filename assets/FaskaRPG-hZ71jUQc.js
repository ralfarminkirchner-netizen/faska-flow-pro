...
//#region src/components/games/engines/FaskaRPG/FaskaRPG.jsx
...
function FaskaRPG({ onExit }) {
	...
	const [obstacles, setObstacles] = (0, import_react.useState)([
		{
			x: 4,
			y: 4
		},
	...
	const [logs, setLogs] = (0, import_react.useState)([
		"Welcome to FaskaRPG!",
		"Click adjacent tiles to move or attack.",
		"Defeat all enemies to win."
	]);
	const [turn, setTurn] = (0, import_react.useState)("player");
	...
	(0, import_react.useEffect)(() => {
		if (turn === "enemy" && player.hp > 0) setTimeout(() => {
			if (enemies.length === 0) {
			...
			newEnemies.forEach((enemy) => {
				if (Math.abs(enemy.x - player.x) + Math.abs(enemy.y - player.y) === 1) {
					...
					let nx = enemy.x;
					let ny = enemy.y;
					if (Math.random() > .5 && dx !== 0) nx += dx;
					else if (dy !== 0) ny += dy;
					else nx += dx;
					if (!(nx === player.x && ny === player.y || newEnemies.some((e) => e.id !== enemy.id && e.x === nx && e.y === ny) || obstacles.some((o) => o.x === nx && o.y === ny)) && nx >= 0 && nx < GRID_SIZE && ny >= 0 && ny < GRID_SIZE) {
						enemy.x = nx;
						enemy.y = ny;
					}
				}
			...
			if (newPlayerHp <= 0) log("You have fallen in battle. Game Over.");
			else setTurn("player");
		}, 600);
	}, [
		turn,
	...
	for (let y = 0; y < GRID_SIZE; y++) for (let x = 0; x < GRID_SIZE; x++) {
		...
		if (isPlayer) {
			...
			content = "👹";
		} else if (isObstacle) Object.assign(cellStyle, STYLES.obstacle);
		if (turn === "player" && !isPlayer && !isObstacle && player.hp > 0) {
			if (Math.abs(player.x - x) + Math.abs(player.y - y) === 1) if (enemyHere) cellStyle.boxShadow = "inset 0 0 15px rgba(233, 69, 96, 0.8)";
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						style: { color: player.hp > 30 ? "#4ecca3" : "#e94560" },
						children: [
							"HP: ",
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						style: { color: turn === "player" ? "#4ecca3" : "#e94560" },
						children: turn === "player" ? "Player's Turn" : "Enemy's Turn..."
					})
				]
			}),
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				ref: logRef,
				children: logs.map((l, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					style: {
						...STYLES.logEntry,
						color: l.includes("Damage") || l.includes("defeated") ? "#e94560" : l.includes("Victory") ? "#4ecca3" : "#aaa"
					},
					children: ["> ", l]
				}, i))
			})
