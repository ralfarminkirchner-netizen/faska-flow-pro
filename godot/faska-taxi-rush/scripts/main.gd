...
const MAX_DAMAGE := 6
const LESSONS := ["WORTART", "SATZ", "ORT"]
...
func spawn_passenger() -> void:
	current_task = WORD_TASKS[int(randi() % WORD_TASKS.size())]
	passenger_onboard = false
	passenger_pos = Vector2(
		260.0 + randf() * (WORLD_SIZE.x - 520.0),
		240.0 + randf() * (WORLD_SIZE.y - 480.0)
	)
...
func normal_destinations() -> Array:
	return [
		{"label": "Bahnhof", "pos": Vector2(320.0, 310.0), "color": Color.html("#38bdf8")},
		{"label": "Schule", "pos": Vector2(2350.0, 330.0), "color": Color.html("#facc15")},
		{"label": "Bibliothek", "pos": Vector2(410.0, 1540.0), "color": Color.html("#a78bfa")},
		{"label": "Park", "pos": Vector2(2300.0, 1490.0), "color": Color.html("#22c55e")},
	]

func word_kind_destinations() -> Array:
	return [
		{"label": "Nomen", "pos": Vector2(310.0, 320.0), "color": Color.html("#38bdf8")},
		{"label": "Verb", "pos": Vector2(2380.0, 320.0), "color": Color.html("#facc15")},
		{"label": "Adjektiv", "pos": Vector2(330.0, 1510.0), "color": Color.html("#a78bfa")},
		{"label": "Praeposition", "pos": Vector2(2350.0, 1510.0), "color": Color.html("#22c55e")},
		{"label": "Konjunktion", "pos": Vector2(1400.0, 910.0), "color": Color.html("#fb923c")},
	]

func sentence_destinations() -> Array:
	return [
		{"label": "Wer/Was", "role": "WER", "pos": Vector2(300.0, 320.0), "color": Color.html("#38bdf8")},
		{"label": "Tut was", "role": "TUT", "pos": Vector2(2380.0, 320.0), "color": Color.html("#facc15")},
		{"label": "Wie/Wo", "role": "WIE", "pos": Vector2(330.0, 1510.0), "color": Color.html("#a78bfa")},
		{"label": "Lagewort", "role": "WO", "pos": Vector2(2350.0, 1510.0), "color": Color.html("#22c55e")},
		{"label": "Bindewort", "role": "BIND", "pos": Vector2(1400.0, 910.0), "color": Color.html("#fb923c")},
	]
...
func spawn_traffic() -> void:
	traffic.clear()
	for i in range(22):
		var lane_y := 250.0 + float(i % 7) * 235.0
...
func spawn_pickups() -> void:
	...
	for i in range(18):
		pickups.append({
			"pos": Vector2(260.0 + randf() * (WORLD_SIZE.x - 520.0), 260.0 + randf() * (WORLD_SIZE.y - 520.0)),
...
func update_world(delta: float) -> void:
	...
	for i in range(traffic.size()):
		...
		if player_is_driving and Vector2(traffic_car["pos"]).distance_to(car_pos) < 56.0 and traffic_car["hit_cooldown"] <= 0.0 and crash_cooldown <= 0.0:
...
func _finish_delivery() -> void:
	...
	score += 260 + time_bonus + drift_bonus + streak * 80
	streak += 1
	deliveries += 1
	if mode_learn:
		learn_hits += 1
		_set_message("Richtig: %s -> %s. Lernserie %d." % [current_task["word"], destination["label"], learn_hits])
	else:
		_set_message("%s abgeliefert: %s. Serie x%d." % [current_task["word"], destination["label"], streak])
	spawn_passenger()
...
func draw_city() -> void:
	...
	var horizontal_start := -fmod(camera.y, 235.0) - 235.0
	for y in range(int(size.y / 235.0) + 4):
		var road_y := horizontal_start + float(y) * 235.0
	...
	var vertical_start := -fmod(camera.x, 320.0) - 320.0
	for x in range(int(size.x / 320.0) + 4):
		var road_x := vertical_start + float(x) * 320.0
...
func draw_passenger() -> void:
	...
	draw_string(font, p + Vector2(-58, -42), current_task["word"], HORIZONTAL_ALIGNMENT_CENTER, 116.0, 17, Color.html("#f8fafc"))
...
func draw_hud() -> void:
	var font := get_theme_default_font()
	draw_rect(Rect2(12, 12, 580, 136), Color(0.02, 0.05, 0.09, 0.80), true)
	...
	draw_string(font, Vector2(26, 92), "Mode %s  Fach %s  Lernziel %d  Fehler %d" % ["Learncade" if mode_learn else "Normal", LESSONS[lesson_index], learn_hits, mistakes], HORIZONTAL_ALIGNMENT_LEFT, -1, 15, Color.html("#cbd5e1"))
	draw_string(font, Vector2(26, 120), _objective_text(), HORIZONTAL_ALIGNMENT_LEFT, -1, 14, Color.html("#fde68a"))
	draw_rect(Rect2(616, 22, 210, 14), Color.html("#111827"), true)
	draw_rect(Rect2(616, 22, 210.0 * boost / 100.0, 14), Color.html("#22c55e"), true)
	draw_string(font, Vector2(616, 58), "WASD/Pfeile  Space Drift  Shift Boost", HORIZONTAL_ALIGNMENT_LEFT, -1, 14, Color.html("#cbd5e1"))
	draw_string(font, Vector2(616, 82), "L Learncade  C Fach  R Neustart", HORIZONTAL_ALIGNMENT_LEFT, -1, 14, Color.html("#94a3b8"))
...
func _mode_message() -> String:
	if mode_learn:
		return "Learncade: Wortarten, Satzstellen und Lese-Orte. C wechselt das Fach."
