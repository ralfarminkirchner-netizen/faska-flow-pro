...
//#region src/components/games/engines/FaskaCoreArcade/FaskaCoreArcade.jsx
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
var import_jsx_runtime = require_jsx_runtime();
var W = 960;
var H = 540;
var STORAGE_KEY = "faska-core-arcade-best-v1";
var GAMES = [
	{
		id: "taxi",
		label: "Taxi Rush",
		icon: "🚕",
		hint: "Fahren, abholen, richtig abliefern."
	},
	{
		id: "snake",
		label: "Snake Trail",
		icon: "🐍",
		hint: "Wachsen, ausweichen, Ziel einsammeln."
	},
	{
		id: "breakout",
		label: "Breakout Grid",
		icon: "🧱",
		hint: "Paddle, Ball, klare Treffer."
	},
	{
		id: "shooter",
		label: "Star Defender",
		icon: "🚀",
		hint: "Zielen, schiessen, ausweichen."
	},
	{
		id: "maze",
		label: "Maze Runner",
		icon: "🧭",
		hint: "Wege lesen, Ausgang finden."
	},
	{
		id: "runner",
		label: "Lane Runner",
		icon: "🏁",
		hint: "Spurwechsel, Tempo, Reaktion."
	}
];
var SUBJECTS = {
	deutsch: {
		...
		challenges: [
			...
			{
				prompt: "Welcher Artikel passt zu \"Baum\"?",
				answer: "Der",
				options: [
					"Der",
					"Die",
			...
			{
				prompt: "Welche Silbe beginnt \"Trommel\"?",
				answer: "Trom",
				options: [
					"Trom",
					"mel",
	...
	sach: {
		...
		challenges: [
			...
			{
				prompt: "Welches Tier ist nachtaktiv?",
				answer: "Eule",
				options: [
					"Eule",
					"Biene",
	...
	musik: {
		...
		challenges: [
			...
			{
				prompt: "Welches Instrument wird geschlagen?",
				answer: "Trommel",
				options: [
					"Trommel",
					"Floete",
...
var rectHit = (a, b) => a.x < b.x + b.w && a.x + a.w > b.x && a.y < b.y + b.h && a.y + a.h > b.y;
var dist = (a, b) => Math.hypot(a.x - b.x, a.y - b.y);
var choose = (items, index) => items[index % items.length];
var shuffle = (items, seed) => [...items].map((item, index) => ({
	item,
	key: (index * 73 + seed * 41 + String(item).length * 19) % 997
...
var drawLabelBox = (ctx, box, text, color, active = false) => {
	ctx.save();
...
var createBaseState = (gameId, variant, subject, level, score) => {
	...
	if (gameId === "taxi") {
		...
		state.pickup = {
			...
			r: 28,
			taken: false,
			label: variant === "learn" ? "Frage" : "Gast"
		};
		state.targets = (variant === "learn" ? challenge.options : [
			"Museum",
			"Park",
			"Bahnhof",
			"Schule"
	...
	if (gameId === "snake") {
		...
		state.food = (variant === "learn" ? challenge.options : [
			"★",
			"●",
			"◆",
			"■"
	...
	if (gameId === "breakout") {
		...
		state.bricks = (variant === "learn" ? challenge.options : [
			"A",
			"B",
			"C",
			"D",
	...
	if (gameId === "maze") {
		...
		state.exits = (variant === "learn" ? challenge.options : [
			"Nord",
			"Ost",
			"Sued",
			"West"
	...
	if (gameId === "runner") {
		...
		state.gates = (variant === "learn" ? challenge.options : [
			"Boost",
			"Ring",
			"Stern",
			"Punkt"
...
function FaskaCoreArcade({ onExit, isLearncade = false }) {
	...
	const [gameId, setGameId] = (0, import_react.useState)("taxi");
	const [variant, setVariant] = (0, import_react.useState)(() => isLearncade ? "learn" : "arcade");
	...
	const activeGame = (0, import_react.useMemo)(() => GAMES.find((game) => game.id === gameId) || GAMES[0], [gameId]);
	...
	(0, import_react.useEffect)(() => {
		...
		const updateTaxi = (state, dt, input) => {
			...
			if (state.pickup.taken) {
				const hit = state.targets.find((target) => rectHit({
					x: car.x - 20,
					y: car.y - 14,
					w: 40,
					h: 28
		...
		const updateSnake = (state, dt, input) => {
			if (input.left && state.dir.x !== 1) state.nextDir = {
				x: -1,
				y: 0
			};
			if (input.right && state.dir.x !== -1) state.nextDir = {
				x: 1,
				y: 0
			};
			if (input.up && state.dir.y !== 1) state.nextDir = {
				x: 0,
				y: -1
			};
			if (input.down && state.dir.y !== -1) state.nextDir = {
				x: 0,
				y: 1
			};
			state.stepTimer += dt;
			if (state.stepTimer < Math.max(.075, .16 - state.level * .006)) return;
			state.stepTimer = 0;
			...
			if (food) completeTarget(state, food.correct, food.x * state.cell + 12, food.y * state.cell + 12);
		...
		const updateBreakout = (state, dt, input) => {
			...
			const brick = state.bricks.find((item) => item.alive && rectHit({
				x: ball.x - ball.r,
				y: ball.y - ball.r,
				w: ball.r * 2,
				h: ball.r * 2
			...
			if (state.variant === "arcade" && state.bricks.every((brickItem) => !brickItem.alive)) completeTarget(state, true, W / 2, 180);
		};
		...
		const updateRunner = (state, dt, input) => {
			...
			state.gates.forEach((gate) => {
				...
				if (gate.lane === player.lane && Math.abs(gate.y - player.y) < 38) completeTarget(state, gate.correct, state.lanes[gate.lane], gate.y);
			});
		};
		...
		const drawBack = (state) => {
			...
			if (state.flash > 0) {
				ctx.fillStyle = `rgba(251, 113, 133, ${Math.min(.28, state.flash)})`;
		...
		const drawSnake = (state) => {
			state.food.forEach((food) => {
				drawLabelBox(ctx, {
					x: food.x * state.cell + 3,
					y: food.y * state.cell + 3,
					w: state.cell * 3,
					h: state.cell - 5
				}, food.label, SUBJECTS[state.subject].color, food.correct);
			});
		...
		const drawMaze = (state) => {
			for (let y = 0; y < state.mazeRows; y += 1) for (let x = 0; x < state.mazeCols; x += 1) if (mazeWall(x, y)) {
				ctx.fillStyle = "#0f766e";
				drawRoundRect(ctx, x * state.cell, y * state.cell + 72, state.cell - 2, state.cell - 2, 7);
				ctx.fill();
			}
			state.exits.forEach((exit) => drawLabelBox(ctx, {
				x: exit.x * state.cell + 3,
				y: exit.y * state.cell + 76,
				w: state.cell * 3,
				h: state.cell - 8
			}, exit.label, SUBJECTS[state.subject].color, exit.correct));
		...
		const drawRunner = (state) => {
			...
			state.gates.forEach((gate) => drawLabelBox(ctx, {
				x: state.lanes[gate.lane] - 64,
				y: gate.y,
				w: 128,
				h: 50
			}, gate.label, SUBJECTS[state.subject].color, gate.correct));
			ctx.fillStyle = "#facc15";
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "absolute inset-x-0 top-0 z-20 flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-slate-950/88 px-4 py-3 backdrop-blur",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-[10px] font-black uppercase tracking-[.22em] text-white/45",
					children: "Stabile Referenz-Engine"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
					className: "text-2xl font-black leading-none md:text-4xl",
					children: "FASKA Core Arcade"
				})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex flex-wrap items-center gap-2",
					children: [
						Object.entries(MODE_META).map(([id, meta]) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							...
							style: { backgroundColor: variant === id ? meta.color : void 0 },
							children: meta.label
						}, id)),
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							onClick: onExit,
							className: "rounded-2xl bg-rose-500 px-4 py-2 text-sm font-black text-white shadow-lg",
							children: "Beenden"
						})
					]
				})]
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "absolute left-0 top-[76px] z-10 flex h-[calc(100%-76px)] w-52 flex-col gap-2 overflow-y-auto border-r border-white/10 bg-slate-950/72 p-3 backdrop-blur",
				children: GAMES.map((game) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
					type: "button",
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: `mt-1 block text-[10px] font-bold leading-tight ${gameId === game.id ? "text-slate-700" : "text-white/45"}`,
							children: game.hint
						})
					]
				}, game.id))
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "absolute inset-0 flex items-center justify-center pl-52 pt-[76px]",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "relative aspect-[16/9] w-[min(calc(100vw-14rem),calc((100vh-7rem)*1.777))] overflow-hidden rounded-[28px] border border-white/15 bg-black shadow-2xl",
					...
					}), variant === "learn" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "pointer-events-none absolute left-5 right-5 top-20 rounded-2xl border border-white/10 bg-slate-950/72 px-4 py-3 text-center shadow-xl backdrop-blur",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "text-xs font-black uppercase tracking-[.18em] text-white/45",
							children: activeSubject.label
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "text-xl font-black text-white",
							children: hud.prompt || "Frage wird geladen."
						})]
					})]
				})
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "absolute bottom-4 left-56 z-20 grid grid-cols-[48px_48px_48px] gap-2 md:hidden",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {}),
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "absolute bottom-4 right-4 z-20 md:hidden",
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["Leben ", hud.lives] }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "text-white/30",
						children: "|"
					}),
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: ["Best ", best] }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "text-white/30",
						children: "|"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "WASD/Pfeile, Space/Enter" })
				]
			})
		]
	});
...
//#endregion
export { FaskaCoreArcade as default };
