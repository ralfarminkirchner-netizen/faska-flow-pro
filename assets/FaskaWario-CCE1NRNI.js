...
//#region src/components/games/engines/FaskaWario/FaskaWario.jsx
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
var import_jsx_runtime = require_jsx_runtime();
var FaskaWario = ({ onExit }) => {
	const canvasRef = (0, import_react.useRef)(null);
	const [uiState, setUiState] = (0, import_react.useState)("START");
	...
	(0, import_react.useEffect)(() => {
		...
		const MICROGAMES = [
			{
				...
				update: (eng, dt) => {
					...
					if (eng.mouse.clicked) eng.data.targets.forEach((t) => {
						const dx = eng.mouse.x - t.x;
						const dy = eng.mouse.y - t.y;
						if (dx * dx + dy * dy <= t.r * t.r) if (t.correct) eng.win();
						else eng.lose();
					});
				},
				draw: (eng, ctx) => {
			...
			},
			{
				...
				update: (eng, dt) => {
					if (eng.keysJustPressed[" "]) {
						eng.data.pumps++;
						if (eng.data.pumps >= eng.data.targetPumps) eng.win();
					}
				},
				draw: (eng, ctx) => {
					const radius = 30 + Math.min(1, eng.data.pumps / eng.data.targetPumps) * 150;
					ctx.beginPath();
			...
			},
			{
				...
				update: (eng, dt) => {
					...
					if (eng.mouse.clicked || eng.keysJustPressed[" "]) eng.lose();
				},
				draw: (eng, ctx) => {
		...
		const BOSS_GAME = {
			...
			init: (eng) => {
				eng.data.nextNum = 1;
				eng.data.nodes = [];
				for (let i = 1; i <= 5; i++) eng.data.nodes.push({
					num: i,
			...
			update: (eng, dt) => {
				if (eng.mouse.clicked) for (let i = 0; i < eng.data.nodes.length; i++) {
		...
		const onKeyDown = (e) => {
			if (e.repeat) return;
			if (!engine.keys[e.key]) engine.keysJustPressed[e.key] = true;
		...
		const drawBackground = () => {
			...
			for (let i = 0; i < engine.width; i += 50) {
				ctx.beginPath();
				ctx.moveTo(i, 0);
				ctx.lineTo(i, engine.height);
				ctx.stroke();
			}
			for (let i = 0; i < engine.height; i += 50) {
				ctx.beginPath();
				ctx.moveTo(0, i);
				ctx.lineTo(engine.width, i);
				ctx.stroke();
			}
		};
		const drawUI = () => {
			...
			if (engine.state !== "START" && engine.state !== "GAMEOVER" && engine.state !== "WIN_GAME") {
				if (engine.state === "TRANSITION") {
					...
					if (engine.transitionTimer <= 0) {
						...
						if (engine.score === 5) currentMicrogame = BOSS_GAME;
						else currentMicrogame = MICROGAMES[Math.floor(Math.random() * MICROGAMES.length)];
					...
					engine.transitionTimer -= dt;
					if (engine.transitionTimer <= 0) if (engine.lives <= 0) {
				...
				if (engine.state === "TRANSITION") {
					...
					ctx.shadowBlur = 0;
				} else if (engine.state === "PLAYING") currentMicrogame.draw(engine, ctx);
				else if (engine.state === "RESULT") {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [
				...
				uiState === "START" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					style: styles.overlay,
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
							style: styles.overlayTitle,
							children: "Willkommen bei FaskaWario!"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							style: styles.overlayText,
							children: "Bestehe die rasanten Mikro-Spiele. Schaffst du den Boss?"
						}),
				...
				uiState === "WIN_GAME" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					style: styles.overlay,
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
							style: {
							...
							children: "Boss besiegt!"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							style: styles.overlayText,
							children: "Fantastisch, du hast gewonnen!"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
							style: styles.overlayText,
							children: ["Score: ", finalScore]
